...
const users: any[] = [];
// подключение
wsServer.on("connection", function w(ws: any) {
  // подключение пользователя
  const user = { connection: ws };
  users.push(user);
  // Получаем сообщение от клиента
  ws.on("message", function m(message: any) {
    ...
    if (soketdatamessage.id === "home") db_.home_db(users, soketdatamessage);
    ...
    if (soketdatamessage.id === "adminreport") db_.adminreport_db(users);
    if (soketdatamessage.id === "changereport")
      db_.chreport_db(users, soketdatamessage.el);
    if (soketdatamessage.id === "adminuser") db_.adminuser_db(users);
    if (soketdatamessage.id === "admin") {
      db_.adminapplication_db(users);
      db_.adminreport_db(users);
      db_.adminuser_db(users);
    }
    if (soketdatamessage.id === "w1") dataw = soketdatamessage.data;
    if (soketdatamessage.id === "r1") datar = soketdatamessage.data;
    if (soketdatamessage.id === "u1") datau = soketdatamessage.data;
  });
  // выход пользователя
